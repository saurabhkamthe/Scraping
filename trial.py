...
product = 'shoes'     #product you want to scrap the data for
...
div = 'IIdQZO _1SSAGr'       #as the result page is not same for all the products need to change the class name every time
a = '_3dqZjq'                #as the result page is not same for all the products need to change the class name every time

#other than these two class names nothinng needs to change                


d_data = r_data.find_all('div', {'class' : div})                                                                  

a_data = r_data.find_all('a', {'class' : a})     
links_list = []              
for i in d_data:
    name = i.find('a', {'class' : a})
    link = name.get('href')
...
with open('flipcart_scraper_2.json', 'a') as f:
